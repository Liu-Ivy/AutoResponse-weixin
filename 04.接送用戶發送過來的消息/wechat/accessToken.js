...
class Wechat {
  ...
  /**
   * 用來獲取沒有過期的access_token
   * return {promise<any>} 直是access_token
   */
  fetchAccessToken() {
    if (this.access_token && this.expires_in && this.isValidAccessToken(this)) {
      //說明之前保存過access_token,並且他是有效的,直接使用
    ...
    return this.readAccessToken() //the return value here is the value from Promise.resolve(res) below
      ...
      .then(res => {
        //將access_token掛載到this上
        this.access_token = res.access_token;
        this.expires_in = res.expires_in;
        //返回res包裝了一層promise對象（此對象為成功的狀態）
        //是this.readAccessToken()最終得返回值
        return Promise.resolve(res);
      });
  }
}
